...
export function ProjectSwitcher() {
    ...
    const { data, isLoading } = useQuery({
        ...
        enabled: !!orgSlug //condição apra que a query so seja executada se esta orgSlug for um valor truefy
    })
                        ...
                        {currentProject ? (
                            <>
                                <Avatar className="size-5">
                                    <AvatarImage src={currentProject.avatarUrl?.toString()} />
                                    <AvatarFallback />
                                </Avatar>
            ...
            <DropdownMenuContent
                            ...
                            <DropdownMenuItem key={project.id} asChild>
                                <Link href={`/org/${orgSlug}/project/${project.slug}`}>
                                    <Avatar className="mr-2 size-5">
                                        <AvatarImage src={project.avatarUrl?.toString()} />
                                        <AvatarFallback />
                                    </Avatar>
                ...
                <DropdownMenuItem asChild>
                    <Link href={`/org/${orgSlug}/create-project`}>
                        <PlusCircle />
                        <span className="line-clamp-1">Create new project</span>
                    </Link>
                </DropdownMenuItem>
            </DropdownMenuContent>
        </DropdownMenu>
